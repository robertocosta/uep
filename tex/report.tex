...
Il sistema è stato testato trasmettendo lo stesso video usato in
\cite{uep}, codificato dall'encoder di riferimento per lo standard SVC
con parametri simili a quelli usati in \cite{uep}.  Per ottenere una
quantità maggiore di pacchetti da trasmettere, il bitstream H264 del
video codificato è stato ripetuto fino ad ottenere uno stream di 16020
frame.

I risultati ottenuti sono mostrati in figura~\ref{fig:iid}.
%
\begin{figure}[H]
    \centering
...
error rate variando la lunghezza media $E[\#_B]$ di una sequenza di
slot cattivi consecutivi e considerando diversi valori per la
...
overhead del 30 \%, mentre la probabilità stazionaria di errore del
canale è stata fissata a $\pi_B = 10^{-1}$. Da questa e da $E[\#_B]$
sono state ricavate le probabilità di transizione secondo
\eqref{eq:mcparams}.

Testando le dimensioni di blocco $K \in \{500, 1000, 5000\}$ e
variando $E[\#_B] \in [1, 1500]$ sono stati ottenuti i risultati
mostrati in figura~\ref{fig:markovvsk}.
%
\begin{figure}[H]
  \centering
        \makebox[\textwidth][c]{\includegraphics[width=10cm]{../avg/2017-11-06_18-07-35.png}}
  \caption{Packet error rate a livello applicazione ottenuta al
    variare del numero medio di slot cattivi consecutivi.}
  \label{fig:markovvsk}
\end{figure}
%
Osservando la coppia di curve relative al caso $K=1000$ è possibile
notare che, in corrispondenza al caso iid ($E[\#_B] = \frac{1}{1 -
  \pi_B} \approx 1.1$) le PER a livello applicazione coincidono col
grafico del test precedente (figura~\ref{fig:iid}). Man mano che la
correlazione aumenta si può osservare che le due PER tendono ad
avvicinrsi, pur rimanendo entrambe intorno al valore di $\pi_B$.
%
\`E possibile cercare la spiegazione di questo effetto nel fatto che,
più $E[\#_B]$ aumenta, più è probabile perdere interi blocchi di $K$
pacchetti. In questo caso la protezione non uniforme diventa inutile
perchè non è possibile estrarre comunque i pacchetti a priorità più
alta dai blocchi parzialmente decodificati ed entrambe le classi di
priorità subiscono la stessa probabilità di errore.
